...
def load_dataset(
    ...
    train_aug, val_aug = [
                             ...
                             vision.ToTensor(), normalize_01_into_pm1,
                         ], [
                             vision.Resize(mid_reso, interpolation=Inter.BICUBIC),
                             # Resize: resize the shorter edge to mid_reso
    ...
    # build dataset
    train_set = ds.ImageFolderDataset(dataset_dir=os.path.join(data_path, 'train'), decode=True, extensions=[".JPEG", ".png"])
    val_set = ds.ImageFolderDataset(dataset_dir=os.path.join(data_path, 'val'), decode=True, extensions=[".JPEG", ".png"])
