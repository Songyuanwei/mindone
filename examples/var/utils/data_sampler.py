...
class InfiniteBatchSampler(Sampler):
    ...
    def gener_indices(self):
        if self.shuffle:
            ...
            indices = mint.randperm(self.dataset_len).asnumpy()
        else:
            indices = mint.arange(self.dataset_len).asnumpy()
...
class DistInfiniteBatchSampler(InfiniteBatchSampler):
    ...
    def gener_indices(self):
        ...
        if self.shuffle:
            g = ms.Generator()
            g.manual_seed(self.epoch + self.same_seed_for_all_ranks)
            global_indices = mint.randperm(self.dataset_len, generator=g)
            ...
            global_indices = mint.arange(self.dataset_len)
        filling = global_max_p - global_indices.shape[0]
        if filling > 0 and self.fill_last:
            global_indices = mint.cat((global_indices, global_indices[:filling]))
        # global_indices = tuple(global_indices.numpy().tolist())

        seps = mint.linspace(0, global_indices.shape[0], self.world_size + 1, dtype=ms.int32)
