...
class VQVAE(nn.Cell):
    def __init__(
        ...
        self.quant_conv = mint.nn.Conv2d(self.Cvae, self.Cvae, quant_conv_ks, stride=1, padding=quant_conv_ks // 2)
        self.post_quant_conv = mint.nn.Conv2d(self.Cvae, self.Cvae, quant_conv_ks, stride=1, padding=quant_conv_ks // 2)
    ...
    def img_to_idxBl(
        self, inp_img_no_grad: ms.Tensor, v_patch_nums: Optional[Sequence[Union[int, Tuple[int, int]]]] = None
    ) -> List[ms.Tensor]:  # return List[Bl]
        f = self.quant_conv(self.encoder(inp_img_no_grad))
        return self.quantize.f_to_idxBl_or_fhat(f, to_fhat=False, v_patch_nums=v_patch_nums)

    def idxBl_to_img(
        self, ms_idx_Bl: List[ms.Tensor], same_shape: bool, last_one=False
    ...
    def embed_to_img(
        self, ms_h_BChw: List[ms.Tensor], all_to_max_scale: bool, last_one=False
    ) -> Union[List[ms.Tensor], ms.Tensor]:
        if last_one:
            return self.decoder(
    ...
    def img_to_reconstructed_img(
        self, x, v_patch_nums: Optional[Sequence[Union[int, Tuple[int, int]]]] = None, last_one=False
    ) -> List[ms.Tensor]:
