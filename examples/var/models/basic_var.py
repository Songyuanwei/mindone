...
class SelfAttention(nn.Cell):
    ...
    def construct(self, x, attn_bias):
        ...
        attention_mask = None if attn_bias is None else attn_bias.to(dtype=main_type).expand(B, self.num_heads, -1, -1)
        ...
        out = (
            self.attention(
                q.to(dtype=main_type), k.to(dtype=main_type), v.to(dtype=main_type), None, None, None, attention_mask
            )[3]
            .swapaxes(1, 2)
            .view((B, L, C))
        )
