...
class VectorQuantizer2(nn.Cell):
    ...
    def construct(self, f_BChw: ms.Tensor, ret_usages=False) -> Tuple[ms.Tensor, List[float], ms.Tensor]:
        ...
        for si, pn in enumerate(self.v_patch_nums):  # from small to large
            ...
            if self.using_znorm:
                rest_NC = (
                    ops.interpolate(f_rest, size=(pn, pn), mode="area").permute(0, 2, 3, 1).reshape(-1, C)
                    if (si != SN - 1)
    ...
    def f_to_idxBl_or_fhat(
        ...
        for si, (ph, pw) in enumerate(patch_hws):  # from small to large
            ...
            # find the nearest embedding
            z_NC = (
                ops.interpolate(f_rest, size=(ph, pw), mode="area").permute(0, 2, 3, 1).reshape(-1, C)
                if (si != SN - 1)
                else f_rest.permute(0, 2, 3, 1).reshape(-1, C)
            )
            if self.using_znorm:
