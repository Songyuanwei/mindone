...
from ..normalization import FP32LayerNorm, LayerNorm
    ...
    def __call__(
        ...
        if rotary_emb is not None:
            ...
            def apply_rotary_emb(hidden_states: ms.Tensor, freqs: ms.Tensor):
                ...
                x_out = view_as_real(x_rotated * freqs).flatten(3, 4)
        ...
        if encoder_hidden_states_img is not None:
            ...
            key_img = mint.transpose(unflatten(key_img, 2, (attn.heads, -1)), 1, 2)
            value_img = mint.transpose(unflatten(value_img, 2, (attn.heads, -1)), 1, 2)

            hidden_states_img = attn.scaled_dot_product_attention(
                query, key_img, value_img, attn_mask=None, dropout_p=0.0, is_causal=False
            )
            hidden_states_img = mint.transpose(hidden_states_img, 1, 2).flatten(2, 3)
            hidden_states_img = hidden_states_img.type_as(query)

        hidden_states = attn.scaled_dot_product_attention(
            query, key, value, attn_mask=attention_mask, dropout_p=0.0, is_causal=False
        )
        hidden_states = mint.transpose(hidden_states, 1, 2).flatten(2, 3)
