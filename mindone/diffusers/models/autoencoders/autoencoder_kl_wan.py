...
class WanUpsample(mint.nn.Upsample):
    r"""
    ...
    def __init__(self, dim: int, mode: str) -> None:
        ...
        if mode == "upsample2d":
            self.resample = nn.SequentialCell(
                WanUpsample(scale_factor=(2.0, 2.0), mode="nearest-exact"), mint.nn.Conv2d(dim, dim // 2, 3, padding=1)
            )
